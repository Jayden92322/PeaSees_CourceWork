...
function getPartsList() {
    ...
    fetch(url, {
        ...
        if (response.hasOwnProperty("Error")) { //checks if response from the web server has an "Error"
            ...
            let dataHTML = `<table style="width: 50%"><tr><th>Part ID</th><th>Category</th><th>Description</th><th>Price</th></tr>`;
            ...
            for (let item of response.parts) {
                ...
                dataHTML += `<tr id="${item.PartID}"><td style="width: 10%">${item.PartID}</td><td style="width: auto">${item.Category}</td><td style="width: auto">${item.PartDescription}</td><td>£ ${item.Price}.00</td><td>`+option+`</td><td><button onclick="addToBasket(${item.PartID})">Add to basket</button></td></tr>`;
            }
